<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.examquestionName" type="text" placeholder="测评试题名称"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;">
			<view style="font-size: 30rpx;">
				<b>{{item.examquestionId}}</b>
			</view>
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">试题表：{{item.examquestionValue}}</text>
			</view>
			<!--<view class="u-box" style="color: #8799A3;">-->
			<!--</view>-->
		</view>
        <view style="float: right;">
            <text v-if="isAuth('examquestion','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(item.id)"></text>
            <text v-if="isAuth('examquestion','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('examquestion','新增')" class="add-btn" @click="onAddTap()">新增</button>
